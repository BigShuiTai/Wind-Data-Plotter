...
def parse_colormap(data):
    ...
    span = vmax - vmin
    colormap["red"].append((0.0, 0.0, data["r"][0][0] / 255))
    colormap["green"].append((0.0, 0.0, data["g"][0][0] / 255))
    colormap["blue"].append((0.0, 0.0, data["b"][0][0] / 255))
    for t, r, g, b in zip(
        data["temp"][1:-1], data["r"][1:-1], data["g"][1:-1], data["b"][1:-1]
    ):
        rel_t = (t - vmin) / span
    ...
    colormap["red"].append((1.0, data["r"][-1][0] / 255, 0.0))
    colormap["green"].append((1.0, data["g"][-1][0] / 255, 0.0))
    colormap["blue"].append((1.0, data["b"][-1][0] / 255, 0.0))
    return vmin, vmax, colormap


def get_colormap(name):
    data = parse_colormap_data("satima/colormap/" + name.lower() + ".txt")
    if not data:
        return 0
